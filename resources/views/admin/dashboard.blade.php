<div>
        ...
        <section class="p-4 my-6 md:p-8 dark:bg-gray-800 dark:text-gray-100">
            <div class="container grid grid-cols-1 gap-6 m-4 mx-auto md:m-0 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div class="flex overflow-hidden rounded-lg dark:bg-gray-900 dark:text-gray-100">
                    ...
                    @php
                        $sum=0;
                        for($i=0;$i<count($sum_solds);$i++){
                          $total_price=$sum_solds[$i]['price_at']*$sum_solds[$i]['quantity'];
                          $sum+=$total_price;
                        }
                    @endphp
                 
                    <div class="flex items-center justify-between flex-1 p-3">
                        <p class="text-2xl font-semibold">{{ number_format($sum,0,'.','.') }} USD</p>
                        <p>All Money</p>
                    </div>
                </div>
                <div class="flex overflow-hidden rounded-lg dark:bg-gray-900 dark:text-gray-100">
                ...
                <div class="flex overflow-hidden rounded-lg dark:bg-gray-900 dark:text-gray-100">
                    ...
                    @php
                        $sum_sold_today=0;
                        for($i=0;$i<count($sum_solds_today);$i++){
                          $sum_today=$sum_solds_today[$i]['price_at']*$sum_solds_today[$i]['quantity'];
                          $sum_sold_today+=$sum_today;
                        }
                    @endphp
                    <div class="flex items-center justify-between flex-1 p-3">
                        <p class="text-2xl font-semibold">{{ number_format($sum_sold_today,0,'.','.') }} USD</p>
                        <p>All Money Today</p>
                    </div>
                </div>
                <div class="flex overflow-hidden rounded-lg dark:bg-gray-900 dark:text-gray-100">
    ...
    <div class="w-full  overflow-hidden rounded-lg shadow-xs">
        <div class="w-full overflow-x-auto">
          <table class="w-full whitespace-no-wrap px-6">
            <thead>
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              ...
              <tr class="text-gray-700 dark:text-gray-400 text-center items-center">
                ...
                <td class="px-4 py-3 text-sm">
                    {!! DNS1D::getBarcodeSVG("$sold->stock_id", 'C128',1,33,'#BDC9C9', true) !!}
                  ...
                  <td class="px-4 py-3 text-sm">
                    {{ number_format( $sold->price_at * $sold->quantity)}} USD
                  </td>
                ...
                <td class="px-4 py-3 ">
                  <div class="flex float-right space-x-4 text-sm">
                    ...
                    <button wire:click.prevent="DeleteRow('{{ $sold->id }}')" title="do you want to delete {{ $sold->stock->name }}" class="flex  sound items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
